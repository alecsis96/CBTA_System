...
export type OfficialRocLine = {
  code: string
  name: string
  amount: number
}

export type OfficialRocPayload = {
  rocNumber: string
  fullName: string
  identifier: string
  address: string
...
const FIRST_EXTRA_TEMPLATE_ROW = 69
...
function applyOfficialRocPayload(xml: string, payload: OfficialRocPayload, rowOffset: number) {
  ...
  DETAIL_ROW_OFFSETS.forEach((detailRowOffset, index) => {
    ...
    writeNumber(`D${detailRowOffset}`, 1)
    writeString(`E${detailRowOffset}`, line.code)
    writeString(`F${detailRowOffset}`, line.name)
    writeNumber(`J${detailRowOffset}`, line.amount)
    writeNumber(`N${detailRowOffset}`, line.amount)
  })
...
function stripExtraRocCopies(sheetXml: string) {
  let nextXml = sheetXml.replace(/<row r="(\d+)"[^>]*>[\s\S]*?<\/row>/g, (match, rowNum) => {
    return parseInt(rowNum, 10) >= FIRST_EXTRA_TEMPLATE_ROW ? '' : match
  })

  nextXml = nextXml.replace(/<mergeCells([^>]*)>([\s\S]*?)<\/mergeCells>/g, (_match, attrs, body) => {
    const mergeCells = body.replace(/<mergeCell ref="([A-Z]+)(\d+):([A-Z]+)(\d+)"\/>/g, (mergeMatch: string, _startCol: string, startRow: string) => {
      return Number(startRow) >= FIRST_EXTRA_TEMPLATE_ROW ? '' : mergeMatch
    })
    const count = (mergeCells.match(/<mergeCell /g) ?? []).length
    return count > 0 ? `<mergeCells${attrs.replace(/\s+count="\d+"/, ` count="${count}"`)}>${mergeCells}</mergeCells>` : ''
  })
...
function createPayloadPages(payloads: OfficialRocPayload[]) {
  const pages: Array<[OfficialRocPayload, OfficialRocPayload | null]> = []
...
export async function buildOfficialRocWorkbookBuffer(templatePath: string, payloads: OfficialRocPayload[]) {
  if (payloads.length === 0) {
    throw new Error('No hay ROC para exportar en el periodo seleccionado.')
  }

  if (!fs.existsSync(templatePath)) {
    throw new Error('No se encontro la plantilla oficial roc 2026.xlsx.')
  }
  ...
  const cleanBaseSheetXml = stripExtraRocCopies(baseSheetXml)
  const payloadPages = createPayloadPages(payloads)

  for (let index = 0; index < payloadPages.length; index += 1) {
    const [topPayload, bottomPayload] = payloadPages[index]
    let sheetXml = clearOfficialRocPayload(cleanBaseSheetXml, TOP_BLOCK_OFFSET)
